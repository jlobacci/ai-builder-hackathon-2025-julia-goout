...
export const Sidebar: React.FC = () => {
  ...
  const pendingCount = user 
    ? MOCK_CONNECTIONS.filter(c => c.target_id === user.id && c.status === 'pendente').length 
    : 0;
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 border-r bg-sidebar-background flex flex-col">
      <div className="p-6"><Logo size="md" /></div>
      <nav className="flex-1 px-3">
        <ul className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              <li key={item.to}>
                <NavLink to={item.to}
                  className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${isActive ? 'bg-sidebar-primary text-sidebar-primary-foreground' : 'text-sidebar-foreground hover:bg-sidebar-accent'}`}>
                  <Icon className="w-5 h-5" />
                  <span className="font-medium">{item.label}</span>
                  {'badge' in item && item.badge && item.badge > 0 && (
                    <Badge variant="destructive" className="ml-auto h-5 min-w-5 px-1.5 text-xs">{item.badge}</Badge>
                  )}
                </NavLink>
              </li>
            );
        ...
        <NavLink to="/about"
          className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${isActive ? 'bg-sidebar-primary text-sidebar-primary-foreground' : 'text-sidebar-foreground hover:bg-sidebar-accent'}`}>
          <Info className="w-5 h-5" /><span className="font-medium">Sobre & Segurança</span>
        </NavLink>
      </nav>
    </aside>
  );
