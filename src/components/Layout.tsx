...
import { 
  Home, Calendar, PlusCircle, MessageCircle, User, Shield,
  Settings, LogOut, Menu, X, Bell, Search, Compass
} from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const profile = user ? MOCK_PROFILES.find(p => p.user_id === user.id) || null : null;
                ...
                return (
                  <NavLink
                    ...
                    className={`flex flex-col items-center px-3 py-2 rounded-lg transition-all ${
                      active ? 'text-[hsl(var(--primary))]' : 'text-muted-foreground hover:text-foreground hover:bg-accent'
                    }`}
                  >
                  ...
                  {profile && (
                    <div className="flex items-center gap-3 pb-4 border-b">
                      <Avatar className="w-12 h-12">
                        <AvatarImage src={profile.avatar_url || undefined} />
                        <AvatarFallback className="bg-primary text-primary-foreground">{profile.display_name?.[0] || 'U'}</AvatarFallback>
                      </Avatar>
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <button key={item.path} onClick={() => { navigate(item.path); setMobileMenuOpen(false); }}
                        className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${active ? 'bg-primary text-primary-foreground' : 'hover:bg-accent'}`}>
                        <Icon className="w-5 h-5" /><span className="font-medium">{item.label}</span>
                      </button>
                    );
                  })}
                  <div className="pt-4 border-t">
                    <button onClick={handleSignOut} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-destructive/10 text-destructive w-full">
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-8">
          {isProfilePage ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
              ...
              <main className="min-h-[calc(100vh-8rem)] max-w-[700px]">{children}</main>
            </div>
          )}
        </div>
      </div>
